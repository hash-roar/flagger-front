...
Page({
    ...
    drawBasicInfo(result) {
        ...
        if(userBasicData.user_social_trend){
            this.data.preferFlagTime.map((item,index)=>{
                console.log(index);
                console.log(item);
                userBasicData.user_social_trend.map((itemTwo)=>{
                    if(itemTwo === item)
                    {
                        let temIsTap = !userTags[index].data.isUserTap.value;
                        console.log(index);
                        userTags[index].setData({
                            isUserTap:{
                                type:Boolean,
                                value:temIsTap
                              }
    ...
    choosePreferFlag(e) {
        ...
        for (let i = 7; i < 9; i++) {
            childUserTag[i].setData({
                ["innerText.value"]: this.data.preferEnviTime[i - 7].value,
            })
        }
